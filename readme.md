...
> ⚡ **性能亮点**：通过精细化锁粒度控制（Fine-grained Locking）和后台异步自愈机制，在 5,000 次并发插入测试中，本连接池将系统吞吐量从 210 QPS 提升至 **1600 QPS**，性能提升达 **7.6 倍**，有效解决了高并发下的连接建立开销问题。
...
为了保证连接的高可用性，内置了独立的后台巡逻线程（Scanner Thread）：

- **健康检查 (Health Check)**：定期对所有空闲连接执行 `mysql_ping`，自动剔除服务端已关闭或网络断开的死连接。
- **空闲回收 (Idle Connection Cleanup)**：当连接数超过 `initSize` 且连接空闲时间超过 `maxIdleTime` 时，自动释放资源，实现连接池的动态缩容。
- **高性能并发设计**：
  - 采用 **锁粒度优化**，将耗时的网络检测（Ping）操作移出锁外。
  - 使用 **快照遍历** 策略，确保心跳检测期间主业务线程获取连接**零阻塞**。
...
*测试环境：Ubuntu 20.04 / 4 Cores / 8GB RAM*
*测试数据：5,000 次连续 `INSERT` 操作*

### 阶段一：基础性能 (无心跳检测)

| 测试场景 | 耗时 (Time) | QPS (Queries/Sec) | 提升倍数 |
| :--- | :--- | :--- | :--- |
| **标准连接 (无连接池)** | 24.621 s | 203 | 1x (基准) |
| **单线程 (使用连接池)** | 12.880 s | 388 | ~1.9x |
| **4线程并发 (使用连接池)** | 4.059 s | 1231 | **~6.1x** |

### 阶段二：优化后性能 (加入细粒度锁心跳检测)

加入心跳检测与锁优化后，性能进一步提升：

| 测试场景 | 耗时 (Time) | QPS (Queries/Sec) | 提升倍数 |
| :--- | :--- | :--- | :--- |
| **标准连接 (无连接池)** | 23.744 s | 210 | 1x (基准) |
| **单线程 (使用连接池)** | 10.667 s | 468 | ~2.2x |
| **4线程并发 (使用连接池)** | 3.125 s | 1600 | **~7.6x** |

> **🚀 结论**: 在并发环境下，连接池展现了巨大的性能优势。通过锁粒度优化，即使开启了后台心跳检测，系统吞吐量不降反升，QPS 最终达到 **1600**，相比原生连接提升超过 **7 倍**。
